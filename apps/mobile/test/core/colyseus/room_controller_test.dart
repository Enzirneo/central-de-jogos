...
void main() {
  ...
  test('conecta e identifica o próprio jogador como host', () {
    handle.emit(ServerEvents.lobbyState, lobbyState());
  ...
  test('acompanha a fase da sala', () {
    handle.emit(ServerEvents.lobbyState, lobbyState(phase: 'starting'));
  ...
  test('queda não-limpa dispara reconexão e re-liga', () async {
    ...
    nova.emit(ServerEvents.lobbyState, lobbyState());
    expect(snap().code, 'ABCD');
  });
